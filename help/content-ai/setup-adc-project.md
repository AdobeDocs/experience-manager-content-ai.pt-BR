...
description: Saiba como configurar um projeto do Adobe Developer Console e autenticar chamadas de API para os Serviços de IA de conteúdo do AEM usando a autenticação de servidor para servidor ou de chave de API.
topic: Configuration
role: Developer, Admin
level: Beginner
solution: Experience Manager
keywords: IA de conteúdo do AEM, Adobe Developer Console, autenticação, servidor para servidor, chave de API, token de acesso
...
* Você foi adicionado como um **Desenvolvedor** no perfil de produto dos Serviços de IA de conteúdo do AEM no **Adobe Admin Console**. Sem essa função, o cartão de API dos **[!UICONTROL Serviços de IA de conteúdo do AEM]** aparece desabilitado e a opção de autenticação de **[!UICONTROL Servidor para servidor]** fica oculta.
...
* Você tem a função de **[Administrador do sistema](https://experienceleague.adobe.com/p-br/docs/support-resources/adobe-support-tools-guide/adobe-admin-console/admin-roles)** no Admin Console do programa. Essa função permite gerenciar perfis de produtos e atribuir usuários ao ambiente.

## Escolha um método de autenticação {#choose-auth}

Os Serviços de IA de conteúdo do AEM são compatíveis com dois métodos de autenticação. Escolha o que corresponde à sua integração:

| Método | Melhor para |
| --- | --- |
| [Servidor para servidor](#s2s-auth) | Serviços de back-end que chamam a API sem interação com o usuário. Retorna um token de acesso de vida curta. |
| [Chave da API](#api-key-auth) | Integrações no lado do cliente ou baseadas em navegador que chamam a API diretamente. Retorna uma chave de longa vida com escopo para domínios permitidos. |

## Autenticação de servidor para servidor {#s2s-auth}

1. Selecione **[!UICONTROL APIs e serviços]** e, em seguida, **[!UICONTROL APIs]**.

   ![Developer Console mostrando APIs e serviços](../assets/e2e-env-setup-28.png)

1. Filtre por **Serviços de IA de conteúdo do AEM** e selecione **[!UICONTROL Criar projeto]** para iniciar um novo projeto ou **[!UICONTROL Adicionar API]** se estiver adicionando o serviço a um projeto existente.
...
1. Se necessário, renomeie a credencial. Selecione **[!UICONTROL Próximo]**.
   ...
   ![Etapa do Adobe Developer Console para renomear a nova credencial Servidor para servidor antes de selecionar Próximo](../assets/e2e-env-setup-30.png)
...
1. Inclua a chave em cada solicitação de API:
   ...
   Seu projeto está pronto. Use a chave em cada solicitação para acessar os Serviços de IA de conteúdo do AEM.
